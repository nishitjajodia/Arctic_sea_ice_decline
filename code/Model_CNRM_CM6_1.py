...
oht_hist, oht_ssp370, aht_hist, aht_ssp370, phi_hist, phi_ssp370, tas_hist, tas_ssp370,oht,aht,phi,tas= import_CNRM_CM6_1()
...
lat_vary_toggle=np.array([1,1,0])  # oht,aht,tas | 0=off, 1=on
avg_p=20 # years
fig1, ax = plt.subplots(5,2,figsize=(20, 30))
ax = ax.ravel()
for i in range(10):
    ref_lat_want=65 + lat_vary_toggle*(-8 + 2*i)  # 1x3 matrix | oht, aht, tas | 55 to 74
    corr_o_phi, corr_a_phi, corr_o_a, corr_t_phi= lead_lag_corr(avg_p,ref_lat_want,oht_hist,aht_hist,tas_hist,phi,oht,aht,tas,'CNRM-CM6-1')
...
plt.tight_layout()
# # plt.savefig('UKESM0_1_LL_refcary_o_a.png',dpi=500)
#%% Maxx corr vs 
corr_ref_lat_plots(57,73,max_corr,max_corr_lag,'CNRM-CN6-1_corr_refcary_o_a.png',savefig=0)  
#%%
...
ax.set_ylim(65,90)
ax.set_title('$\phi$ vs Time (year)- UKESM1-0-LL')
ax.legend()
# directory='plots'
# plots_path = os.path.join(directory,'iel_time_plot_CNRM-CM6-1')
