...
for j in range(len(ref_lat_range)):
    j_lat[j,3] = np.argmin(abs(np.array(cnrmcm16_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,0] = np.argmin(abs(np.array(can_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,1] = np.argmin(abs(np.array(ukesm_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,2] = np.argmin(abs(np.array(mpiesmHR_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))\
...
ax=ax.ravel()
fontsize=18
plt.suptitle('Correlation maps for $r[\Delta AHT,\Delta \phi]$',fontsize=fontsize+2,y=0.93)
for model in range(4):
    ...
    for j in range(len(ref_lat_range)):
        for i in range(-avg_p,avg_p+1):
            delta_aht=np.average(aht_data[model] [151+i:151+avg_p+i,:,j_lat[j,model ]],axis=0) -np.average(aht_data[model][150-avg_p+i:150+i,:,j_lat[j,model ]],axis=0)
            pearsonr_corr[j,i+avg_p,model],p_values[j,i+avg_p,model]=stats.pearsonr(delta_aht,delta_phi_fixed)
...
#%%
directory='plots'
plots_path = os.path.join(directory,'pearsonr_pvalue_modelcombined_aht_phi.pdf')
