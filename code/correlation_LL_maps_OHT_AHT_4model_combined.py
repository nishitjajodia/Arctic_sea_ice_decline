...
#%% ref lats
ref_lat_range=np.linspace(50,87,20)
...
j_lat=np.zeros([len(ref_lat_range),4,2])            #lats x models x variable
extent=[-avg_p,avg_p, ref_lat_range[0],ref_lat_range[-1]]
#%% ref lat indices
for j in range(len(ref_lat_range)):
    j_lat[j,3,0] = np.argmin(abs(np.array(cnrmcm16_oht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,0,0] = np.argmin(abs(np.array(can_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
    j_lat[j,1,0] = np.argmin(abs(np.array(ukesm_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
    j_lat[j,2,0] = np.argmin(abs(np.array(mpiesmHR_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
    j_lat[j,3,1] = np.argmin(abs(np.array(cnrmcm16_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,0,1] = np.argmin(abs(np.array(can_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,1,1] = np.argmin(abs(np.array(ukesm_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
    j_lat[j,2,1] = np.argmin(abs(np.array(mpiesmHR_aht_hist.variables["ref_lat_n"]) - ref_lat_range[j]))
...
ax=ax.ravel()
fontsize=18
plt.suptitle('Correlation maps for $r[\Delta OHT,\Delta AHT]$',fontsize=fontsize+2,y=0.93)
for model in range(4):
    
    
    for j in range(len(ref_lat_range)):
        delta_aht_fixed=np.average(aht_data[model] [151:151+avg_p,:,j_lat[j,model,1]],axis=0) -np.average(aht_data[model][150-avg_p:150,:,j_lat[j,model,1]],axis=0)
        for i in range(-avg_p,avg_p+1):
            delta_oht=np.average(oht_data[model] [151+i:151+avg_p+i,:,j_lat[j,model,0]],axis=0) -np.average(oht_data[model][150-avg_p+i:150+i,:,j_lat[j,model,0]],axis=0)
            pearsonr_corr[j,i+avg_p,model],p_values[j,i+avg_p,model]=stats.pearsonr(delta_oht,delta_aht_fixed)
    ...
    if model>=2: ax[model].set_xlabel('Lead time of $\Delta$AHT (Years) ',fontsize=fontsize)
plt.subplots_adjust(hspace=0.1, wspace=0.05)
cbar=fig.colorbar(plot[1], ax=ax, orientation='horizontal', fraction=0.046, pad = 0.08)
cbar.ax.tick_params(labelsize=fontsize-4)
#%%
directory='plots'
plots_path = os.path.join(directory,'pearsonr_pvalue_modelcombined_oht_aht.pdf')
