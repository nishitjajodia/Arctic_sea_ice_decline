...
avg_p=20
#%% datasets
oht_data=[can_oht,ukesm_oht,mpiesmHR_oht]
#%%
lat=[None]*3
lev=[None]*3
msft=[None]*3
...
j_lat=np.zeros([len(ref_lat_range),3,2])            #lats x models
extent=[-avg_p,avg_p, ref_lat_range[0],ref_lat_range[-1]]

for i in range(3):
    for j in range(len(ref_lat_range)):
        j_lat[j,i,0] = np.argmin(abs(np.array(lat[i] - ref_lat_range[j])))
for j in range(len(ref_lat_range)):
    j_lat[j,0,1] = np.argmin(abs(np.array(can_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
    j_lat[j,1,1] = np.argmin(abs(np.array(ukesm_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
    j_lat[j,2,1] = np.argmin(abs(np.array(mpiesmHR_oht_hist.variables["ref_lat"]) - ref_lat_range[j]))
#%% amoc phi plots
...
ax=ax.ravel()
fontsize=18
plt.suptitle('Correlation maps for r[$\Delta$AMOC,$\Delta$OHT]',fontsize=fontsize+2,y=0.92)
for model in range(3):
    
    delta_oht_fixed=np.average(oht_data[model] [151:151+avg_p,:,j_lat[j,model,1]],axis=0) -np.average(oht_data[model][150-avg_p:150,:,j_lat[j,model,1]],axis=0)
    for j in range(len(ref_lat_range)):
        for i in range(-avg_p,avg_p+1):
            delta_amoc=np.average(amoc_index[model] [151+i:151+avg_p+i,j_lat[j,model,0],:],axis=0) -np.average(amoc_index[model][150-avg_p+i:150+i,j_lat[j,model,0],:],axis=0)
            pearsonr_corr[j,i+avg_p,model]=stats.pearsonr(delta_amoc,delta_oht_fixed)[0]
            p_values[j,i+avg_p,model]=stats.pearsonr(delta_amoc,delta_oht_fixed)[1]
    ...
    if model>=2: ax[model].set_xlabel('Lead time of $\Delta$OHT (Years) ',fontsize=fontsize)
plt.subplots_adjust(hspace=0.1, wspace=0.05)
cbar=fig.colorbar(plot[1], ax=ax, orientation='horizontal', fraction=0.1, pad = 0.07)
cbar.ax.tick_params(labelsize=fontsize-4)
#%%
directory='plots'
plots_path = os.path.join(directory,'pearsonr_pvalue_modelcombined_amoc_oht.pdf')
